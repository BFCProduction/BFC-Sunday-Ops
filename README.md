...
- **Workbooks** — a scheduling layer above events for multi-event / multi-day productions (conferences, assemblies). Reachable from the sidebar, mobile nav, and Home; writes are admin-gated. Includes:
  ...
  - **Event manager** — the Events tab prioritizes unattached events within the workbook date range, exposes searchable nearby dates separately, supports multi-select attachment, and explains why events already owned by another workbook are unavailable. New events can be entered manually or linked from a PCO plan, prefilled to the workbook date range, and attached atomically when created.
